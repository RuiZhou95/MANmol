...
def plot_diagonal_validation(y_train, y_train_pred, y_test, y_test_pred):
    ...
    plt.xlabel('MoLFormer calculated log2(AE) [kcal/mol]', font1)
    plt.ylabel('MoLFormer predicted log2(AE) [kcal/mol]', font1)
    plt.xlim((4.5, 9.5))
    plt.ylim((4.5, 9.5))
    ...
    plt.savefig("./figure/MoLFormer-XL.png", dpi=600)

def train_final_model():
    batch_size = 64
    learning_rate = 6.576e-5
    lora_r = 12
    lora_alpha = 19
    dropout_rate = 0.2444
    max_length = 256
    ...
    dataset = SMILESDataset(smiles_list, target_values_normalized, tokenizer, max_length)
    train_idx, val_idx = next(KFold(n_splits=5, shuffle=True, random_state=6600).split(dataset)) # 《《《《《《《《
    ...
    model = MoLFormerModel(AutoModel.from_pretrained(model_name, trust_remote_code=True))
    ...
    for epoch in range(num_epochs):
        ...
        if r2_val - best_val_score > min_delta:
            ...
            patience_counter = 0
            torch.save(model.state_dict(), "./pretrained_model/MoLFormer-XL-AE_best.model")
    ...
    model.load_state_dict(torch.load("./pretrained_model/MoLFormer-XL-AE_best.model"))
    ...
    plot_diagonal_validation(
        y_train=scaler.inverse_transform(np.concatenate([target.cpu().numpy() for _, _, target in train_loader]).reshape(-1, 1)).flatten(),
        y_train_pred=scaler.inverse_transform(np.concatenate([model(input_ids.to(device), attention_mask.to(device)).cpu().detach().numpy() for input_ids, attention_mask, _ in train_loader]).reshape(-1, 1)).flatten(),
        y_test=scaler.inverse_transform(np.concatenate([target.cpu().numpy() for _, _, target in test_loader]).reshape(-1, 1)).flatten(),
        y_test_pred=scaler.inverse_transform(np.concatenate([model(input_ids.to(device), attention_mask.to(device)).cpu().detach().numpy() for input_ids, attention_mask, _ in test_loader]).reshape(-1, 1)).flatten()
    )
