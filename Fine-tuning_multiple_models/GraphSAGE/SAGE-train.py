...
def plot_diagonal_validation(y_train, y_train_pred, y_test, y_test_pred):
    ...
    plt.ylabel('GraphSAGE predicted log2(AE) [kcal/mol]', font1)
    plt.xlim((4.5, 9.5))
    plt.ylim((4.5, 9.5))
    ...
    plt.savefig('./figure/GraphSAGE.png', dpi=600)

def train_final_model():
    batch_size = 32
    learning_rate = 0.002647
    hidden_dim = 192
    dropout_rate = 0.27
    num_layers = 2
    ...
    model = GraphSAGEModel(num_layers=num_layers, hidden_dim=hidden_dim, dropout_rate=dropout_rate).to(device)
    optimizer = torch.optim.Adam(model.parameters(), lr=learning_rate, weight_decay=1e-5)
    criterion = nn.MSELoss()

    kfold = KFold(n_splits=5, shuffle=True, random_state=2945) # 《《《《《《《《《《
    ...
    for epoch in range(2000):
        ...
        if r2_val - best_val_score > min_delta:
            ...
            patience_counter = 0
            torch.save(model.state_dict(), "./pretrained_model/GraphSAGE_best.model")
    ...
    model.load_state_dict(torch.load("./pretrained_model/GraphSAGE_best.model"))
