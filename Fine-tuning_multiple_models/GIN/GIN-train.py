...
class GINModel(nn.Module):
    def __init__(self, num_layers=2, hidden_dim=64, dropout_rate=0.3):
        ...
        nn1 = nn.Sequential(nn.Linear(1, hidden_dim), nn.ReLU(), nn.Linear(hidden_dim, hidden_dim), nn.Dropout(dropout_rate))
        nn2 = nn.Sequential(nn.Linear(hidden_dim, hidden_dim * 2), nn.ReLU(), nn.Linear(hidden_dim * 2, hidden_dim * 2), nn.Dropout(dropout_rate))
...
def plot_diagonal_validation(y_train, y_train_pred, y_test, y_test_pred):
    ...
    plt.ylabel('GIN predicted log2(AE) [kcal/mol]', font1)
    ...
    plt.savefig('./figure/GIN.png', dpi=600)

def train_final_model():
    batch_size = 112 
    learning_rate = 0.00077937
    hidden_dim = 160 
    dropout_rate = 0.292699 
    num_layers = 3
    ...
    model = GINModel(num_layers=num_layers, hidden_dim=hidden_dim, dropout_rate=dropout_rate).to(device)
    optimizer = torch.optim.Adam(model.parameters(), lr=learning_rate, weight_decay=1e-5)
    criterion = nn.MSELoss()

    kfold = KFold(n_splits=5, shuffle=True, random_state=68) # 《《《《《《《《《
    ...
    for epoch in range(2000):
        ...
        if r2_val - best_val_score > min_delta:
            ...
            patience_counter = 0
            torch.save(model.state_dict(), "./pretrained_model/GIN_best.model")
    ...
    model.load_state_dict(torch.load("./pretrained_model/GIN_best.model"))
