...
def plot_diagonal_validation(y_train, y_train_pred, y_test, y_test_pred):
    ...
    plt.ylabel('MPNN predicted log2(AE) [kcal/mol]', font1)
    plt.xlim((4.5, 9.5))
    plt.ylim((4.5, 9.5))
    ...
    plt.savefig('./figure/MPNN.png', dpi=600)
...
def train_final_model():
    ...
    kfold = KFold(n_splits=5, shuffle=True, random_state=517) # 《《《《《《《《《《《
    ...
    for epoch in range(2000):
        ...
        if r2_val - best_val_score > min_delta:
            ...
            patience_counter = 0
            torch.save(model.state_dict(), "./pretrained_model/MPNN_best.model")
    ...
    model.load_state_dict(torch.load("./pretrained_model/MPNN_best.model"))
