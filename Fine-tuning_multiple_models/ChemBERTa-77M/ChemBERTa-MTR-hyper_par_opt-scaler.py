...
def objective(trial):
    ...
    dropout_rate = trial.suggest_float("dropout_rate", 0.15, 0.3) # 0.1, 0.3
    max_length = trial.suggest_categorical("max_length", [128, 256, 512])
    num_epochs = trial.suggest_int("num_epochs", 100, 200) # 100, 500

    # 数据集和KFold
    kf = KFold(n_splits=5, shuffle=True, random_state=82) # 24
    r2_scores = []
    ...
    print(f"Trial {trial.number} finished with value: {mean_r2:.4f} and parameters: {trial.params}.")
    return mean_r2
