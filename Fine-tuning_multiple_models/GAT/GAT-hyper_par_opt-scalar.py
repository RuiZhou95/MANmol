...
target_dict = {i: target_values_normalized[i] for i in range(len(target_values_normalized))}

class GATModel(nn.Module):
    def __init__(self, num_layers=2, hidden_dim=64, heads=4, dropout_rate=0.3):
        super(GATModel, self).__init__()
        self.convs = nn.ModuleList()
        self.convs.append(GATConv(1, hidden_dim, heads=heads, dropout=dropout_rate))
        for _ in range(num_layers - 1):
            self.convs.append(GATConv(hidden_dim * heads, hidden_dim, heads=heads, dropout=dropout_rate))
...
def train(model, train_loader, optimizer, criterion, device):
    ...
    # 反归一化
    all_preds_train = scaler.inverse_transform(all_preds_train.reshape(-1, 1)).flatten()
    all_targets_train = scaler.inverse_transform(all_targets_train.reshape(-1, 1)).flatten()
...
def objective(trial):
    ...
    model = GATModel(num_layers=num_layers, hidden_dim=hidden_dim, heads=heads, dropout_rate=dropout_rate).to(device)
    optimizer = torch.optim.Adam(model.parameters(), lr=learning_rate, weight_decay=1e-5)
    criterion = nn.MSELoss()

    kfold = KFold(n_splits=5, shuffle=True, random_state=256) # 5 random_state=42
    val_r2_scores = []
