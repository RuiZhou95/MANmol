...
class GATModel(nn.Module):
    def __init__(self, num_layers=2, hidden_dim=64, heads=4, dropout_rate=0.3):
        ...
        self.convs.append(GATConv(1, hidden_dim, heads=heads, dropout=dropout_rate))  # 输入维度为1，输出维度为 hidden_dim
...
def plot_diagonal_validation(y_train, y_train_pred, y_test, y_test_pred):
    ...
    plt.ylabel('GAT predicted log2(AE) [kcal/mol]', font1)
    plt.xlim((4.5, 9.5))
    plt.ylim((4.5, 9.5))
    ...
    plt.savefig('./figure/GAT.png', dpi=600)
...
def train_final_model():
    ...
    for epoch in range(2000):
        ...
        if r2_val - best_val_score > min_delta:
            ...
            patience_counter = 0
            torch.save(model.state_dict(), "./pretrained_model/GAT_best.model")
    ...
    model.load_state_dict(torch.load("./pretrained_model/GAT_best.model"))
